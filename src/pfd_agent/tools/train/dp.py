    ...
    def run(self)->Tuple[Path, Path, str]:
        ...
        try:
            ...
            for f in self.train_data:
                # Convert to absolute path before reading
                abs_path = f if f.is_absolute() else (Path(original_cwd) / f)
                train_data= read(str(abs_path),index=":")
            ...
            if self.valid_data:
                abs_valid_path = self.valid_data if self.valid_data.is_absolute() else (Path(original_cwd) / self.valid_data)
                valid_data = DPTrain.ase2multisys(read(str(abs_valid_path), index=":"), labeled=True)
                if mixed_type:
            ...
            if self.optional_files is not None:
                for f in self.optional_files:
                    # Convert optional files to absolute paths
                    abs_opt_path = f if f.is_absolute() else (Path(original_cwd) / f)
                    Path(f.name).symlink_to(abs_opt_path)

            # Convert model_path to absolute path if needed
            abs_model_path = None
            if self.model_path:
                abs_model_path = self.model_path if self.model_path.is_absolute() else (Path(original_cwd) / self.model_path)
            ...
            model_abs_path = output_dir / self.model_file
            log_abs_path = output_dir / self.log_file

            return model_abs_path, log_abs_path, err
        finally:
