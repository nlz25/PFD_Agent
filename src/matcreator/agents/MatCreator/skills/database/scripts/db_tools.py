#!/usr/bin/env python3
"""
Database CLI tools — direct replacements for the data MCP server tools.
...
Commands:
  ...
  query-info       Run a SELECT on the SQLite information data.
  read-structure   Parse structure file(s) and extract chemical compositions.
  query-compounds  Query an ASE data with flexible selection criteria.
  export-entries   Export entries from an ASE data in various formats.

Environment variables (fallbacks for --info-db / --db-path):
  INFO_DB_PATH     Path to the SQLite information data.
  ASE_DB_PATH      Path to an ASE data file.
"""
...
def cmd_save_extxyz(args: argparse.Namespace) -> Dict[str, Any]:
    """Save an extxyz file into an ASE db and register it in the info data."""
...
def cmd_query_info(args: argparse.Namespace) -> Dict[str, Any]:
    """Execute a SELECT on the SQLite information data."""
    sql_code = args.sql
...
def cmd_query_compounds(args: argparse.Namespace) -> Dict[str, Any]:
    """Query an ASE data with flexible selection criteria."""
...
def cmd_export_entries(args: argparse.Namespace) -> Dict[str, Any]:
    """Export entries from an ASE data."""
    mode = args.mode
    db_path = args.db_path
    fmt = args.fmt
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_save.add_argument(
        ...
        help="Path to the SQLite information data (or set INFO_DB_PATH env var)",
    )
    ...
    # ---- query-info ----
    p_qi = sub.add_parser("query-info", help="Run a SELECT on the SQLite information data")
    ...
    # ---- query-compounds ----
    p_qc = sub.add_parser("query-compounds", help="Query an ASE data with flexible selectors")
    p_qc.add_argument(
    ...
    p_qc.add_argument(
        ...
        help="Path to the ASE data (or set ASE_DB_PATH env var)",
    )
    ...
    # ---- export-entries ----
    p_ex = sub.add_parser("export-entries", help="Export entries from an ASE data")
    p_ex.add_argument(
        "--db-path",
        default=os.environ.get("ASE_DB_PATH", ""),
        help="Path to the ASE data (or set ASE_DB_PATH env var)",
